...
function convertDTCCheck(byteA, _byteB, _byteC, _byteD) {
...
function convertDTCRequest(byteA, byteB, byteC, byteD, byteE, byteF) {
    ...
    const decodeDTCCode = (byte1, byte2) => {
        //If 00 00 --> No code.
        ...
        const codeString = firstChar + secondChar + thirdChar + byte2;
        return codeString;
    };
...
function convertFuelTrim(byte) {
    return (100 / 128) * parseInt(byte, 16) - 100;
}
...
function convertTimeSinceEngineStart(byteA, byteB) {
    return parseInt(byteA, 16) * 256 + parseInt(byteB, 16);
}

function convertRelativeThrottlePosition(byte) {
    return parseInt(byte, 16) * (100 / 255);
}

function convertFuelRate(byteA, byteB) {
    return (parseInt(byteA, 16) * 256 + parseInt(byteB, 16)) / 20;
}

function convertAcceleratorPedalPositionF(byteA, byteB) {
    return parseInt(byteA, 16) * 256 + parseInt(byteB, 16);
}

function convertPercentage(byte) {
    (100 / 255) * byte;
}
...
function convertVIN(byte) {
    const splittedByte = byte.split("");
    const tmp = [];
    let vin = "";
    for (const i in splittedByte) {
...
const responsePIDS = [
    ...
    },
    {
        ...
        max: 65535,
        unit: "seconds",
        convertToUseful: convertTimeSinceEngineStart,
    },
    {
        mode: modeRealTime,
        pid: "20",
    ...
    },
    {
        ...
        max: 100,
        unit: "%",
        convertToUseful: convertPercentage,
    },
    {
        mode: modeRealTime,
        pid: "30",
    ...
    },
    {
        ...
        max: 100,
        unit: "%",
        convertToUseful: convertRelativeThrottlePosition,
    },
    {
        mode: modeRealTime,
        pid: "46",
    ...
    },
    {
        ...
        max: 100,
        unit: "%",
        convertToUseful: convertAcceleratorPedalPositionF,
    },
    {
        mode: modeRealTime,
        pid: "4C",
    ...
    },
    {
        ...
        max: 3212.75,
        unit: "L/h",
        convertToUseful: convertFuelRate,
    },
